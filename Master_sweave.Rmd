...
breastGeneList=unique(c(as.character(OncoList$Gene.Symbol[c(grep("breast", OncoList$Tumour.Types.Germline.),grep("breast", OncoList$Tumour.Types.Somatic.) )]),"MYC", "FGFR1", "FGFR2", "KMT2C"))
...
plot(QPs[[8]]$CentroidX, QPs[[8]]$CentroidY, col=qpathPal[QPs[[8]]$CellType], main="example case 5 IDC", pch=".",
...
```{r}
DIn=read.csv("data/Exome_QC.csv")

# barplot of mean target coverage
barplot(DIn$hs_mean_target_coverage, ylab = "average coverage x", col=c(dcisidcCol, 3), names.arg = DIn$sample_alias, las=2)
abline(v=c(3.2, 6.8, 10.4, 14, 17.6)+0.5, lty=2)

# compare the percent target bases
Pct=t(DIn[ ,c(6:11)])
colnames(Pct)=DIn$sample_alias
CNames=sapply(strsplit(rownames(Pct), "_"), function(x) x[length(x)])
Pct=cbind(Pct, as.numeric(substr(CNames, 1, nchar(CNames)-1)))
...
## Abba cohort
APath="data/Abba2015/Abba_CNV_gatk/" #"data/Abba2015/acapseg/" #"~/Downloads/acapseg/" #"data/Abba2015/Abba_CNV_gatk/called_Regions/" # #"data/Abba2015/Abba_CNVs/"  #Abba_CNV_gatk/called_Regions/" # USE THIS ONE?
...
alllvl=unique(unlist(sapply(Mutall2, function(x) levels(x$Variant_Classification))))
